...
export default function CandidatesScreen() {
    ...
    useEffect(() => {
        ...
        else if (filter === 'inactive') list = list.filter(c => c.isActive === false);

        if (searchText && searchText.trim().length > 0) {
            const q = searchText.trim().toLowerCase();
            list = list.filter(c => (c.fullname || '').toLowerCase().includes(q) || String(c.id).includes(q));
        }

        setDisplayed(list);
    }, [filter, allCandidates, searchText]);
